...
-- Readiness has two honest gates. Recent, explicitly bounded coverage may be
-- useful before vendor retention and the first reconciliation sweep have
-- completed; complete readiness may not be claimed until every mandatory
-- check is green. Keep the distinction in the durable expectation registry so
-- workers do not maintain a second hard-coded quality vocabulary.
ALTER TABLE quality.check_expectation
  ADD COLUMN IF NOT EXISTS blocks_partial_readiness boolean NOT NULL DEFAULT true;

UPDATE quality.check_expectation
   SET blocks_partial_readiness=false
 WHERE check_id IN (
   'retention_limit_recorded','webhook_gap_recovered','delete_handling'
 );

COMMENT ON COLUMN quality.check_expectation.blocks_partial_readiness IS
  'True when a missing, failed, or blocked result prevents bounded ready_partial coverage. All blocks_readiness checks still gate ready_complete.';

-- The DDL owner performs this one bounded backfill across existing tenants.
-- Runtime roles never receive an RLS bypass, and FORCE is restored before the
-- transaction can commit.
ALTER TABLE semantic_internal.canonical_transform_commits NO FORCE ROW LEVEL SECURITY;
ALTER TABLE semantic_internal.readiness_projection_outbox NO FORCE ROW LEVEL SECURITY;

ALTER TABLE semantic_internal.canonical_transform_commits
  ADD COLUMN IF NOT EXISTS partial_quality_status text,
  ADD COLUMN IF NOT EXISTS complete_quality_status text;
UPDATE semantic_internal.canonical_transform_commits
   SET partial_quality_status=coalesce(partial_quality_status,quality_status),
       complete_quality_status=coalesce(complete_quality_status,quality_status)
 WHERE partial_quality_status IS NULL OR complete_quality_status IS NULL;
ALTER TABLE semantic_internal.canonical_transform_commits
  ALTER COLUMN partial_quality_status SET DEFAULT 'blocked',
  ALTER COLUMN partial_quality_status SET NOT NULL,
  ALTER COLUMN complete_quality_status SET DEFAULT 'blocked',
  ALTER COLUMN complete_quality_status SET NOT NULL;
ALTER TABLE semantic_internal.canonical_transform_commits
  DROP CONSTRAINT IF EXISTS canonical_transform_commits_partial_quality_status_check,
  DROP CONSTRAINT IF EXISTS canonical_transform_commits_complete_quality_status_check,
  DROP CONSTRAINT IF EXISTS canonical_transform_commits_quality_tier_order_check;
ALTER TABLE semantic_internal.canonical_transform_commits
  ADD CONSTRAINT canonical_transform_commits_partial_quality_status_check
    CHECK (partial_quality_status IN ('passed','warning','failed','blocked')),
  ADD CONSTRAINT canonical_transform_commits_complete_quality_status_check
    CHECK (complete_quality_status IN ('passed','warning','failed','blocked')),
  ADD CONSTRAINT canonical_transform_commits_quality_tier_order_check CHECK (
    array_position(ARRAY['passed','warning','failed','blocked'],complete_quality_status)
      >=array_position(ARRAY['passed','warning','failed','blocked'],partial_quality_status)
  );

ALTER TABLE semantic_internal.readiness_projection_outbox
  ADD COLUMN IF NOT EXISTS partial_quality_status text,
  ADD COLUMN IF NOT EXISTS complete_quality_status text;
UPDATE semantic_internal.readiness_projection_outbox
   SET partial_quality_status=coalesce(partial_quality_status,CASE
         WHEN state='blocked' THEN 'blocked'
         WHEN state='degraded' THEN 'failed'
         WHEN reason_code='quality_warning' THEN 'warning'
         ELSE 'passed'
       END),
       complete_quality_status=coalesce(complete_quality_status,CASE
         WHEN state='blocked' THEN 'blocked'
         WHEN state='degraded' THEN 'failed'
         WHEN reason_code='quality_warning' THEN 'warning'
         ELSE 'passed'
...
ALTER TABLE semantic_internal.readiness_projection_outbox
  ...
  ALTER COLUMN complete_quality_status SET NOT NULL;
ALTER TABLE semantic_internal.readiness_projection_outbox
  DROP CONSTRAINT IF EXISTS readiness_projection_outbox_partial_quality_status_check,
  DROP CONSTRAINT IF EXISTS readiness_projection_outbox_complete_quality_status_check,
  DROP CONSTRAINT IF EXISTS readiness_projection_outbox_quality_tier_order_check;
ALTER TABLE semantic_internal.readiness_projection_outbox
  ADD CONSTRAINT readiness_projection_outbox_partial_quality_status_check
    CHECK (partial_quality_status IN ('passed','warning','failed','blocked')),
  ADD CONSTRAINT readiness_projection_outbox_complete_quality_status_check
    CHECK (complete_quality_status IN ('passed','warning','failed','blocked')),
  ADD CONSTRAINT readiness_projection_outbox_quality_tier_order_check CHECK (
    array_position(ARRAY['passed','warning','failed','blocked'],complete_quality_status)
      >=array_position(ARRAY['passed','warning','failed','blocked'],partial_quality_status)
  );

ALTER TABLE semantic_internal.canonical_transform_commits FORCE ROW LEVEL SECURITY;
ALTER TABLE semantic_internal.readiness_projection_outbox FORCE ROW LEVEL SECURITY;

-- Connector page observations are immutable diagnostics for the exact page
-- that produced them. They deliberately contain page-local unknown warnings,
-- so they are not the current health authority. Topic health is instead
-- derived from the current connection generation's durable stream state. This
-- lets a completed full-history/reconciliation cycle turn green without a
-- later page-local warning overwriting it.
