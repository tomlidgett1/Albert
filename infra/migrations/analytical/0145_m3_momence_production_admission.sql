-- Momence pack 1.0 is a real ingesting connector. Earlier runtime tables and
-- security-definer functions intentionally admitted only the three original
-- connector packs; widening staging alone would therefore fail after the first
-- API page. Admit this exact version through the same reconciliation,
-- capability, immutable-transform and pack-release boundaries.
...
ALTER TABLE quality.connector_check_observation
  ...
  CHECK (connector_id IN ('lightspeed-r','xero','deputy','momence'));
...
INSERT INTO semantic_internal.connector_pack_release (
  ...
  'momence','1.0.0',1,NULL,'active',
  '0145_m3_momence_production_admission.sql',now()
)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
     ...
     WHERE release.connector_id='momence'
    ...
    RAISE EXCEPTION 'Momence connector pack 1.0.0 conflicts with durable release authority';
  END IF;
END
$$;
...
DECLARE predecessor_guard CONSTANT text:=
  'p_connector_id NOT IN (''lightspeed-r'',''xero'',''deputy'')';
...
BEGIN
  ...
  -- security mode and configuration while changing only the closed connector
  -- vocabulary. Resolve only the four reviewed runtime signatures: scanning
  -- pg_proc by schema/name and then calling pg_get_functiondef can evaluate an
  -- aggregate or window entry, for which PostgreSQL deliberately raises.
  FOREACH function_oid IN ARRAY required_functions LOOP
    IF NOT EXISTS (
      SELECT 1 FROM pg_proc procedure
       WHERE procedure.oid=function_oid AND procedure.prokind='f'
    ) THEN
      RAISE EXCEPTION 'Momence connector admission target is not an ordinary function: %',
    ...
    IF position(predecessor_guard IN original_definition)=0 THEN
      RAISE EXCEPTION 'Momence connector admission target lacks the exact predecessor guard: %',
        function_oid USING ERRCODE='55000';
    END IF;
    ...
    IF patched_definition=original_definition THEN
      RAISE EXCEPTION 'Momence connector function admission made no change for %',
        function_oid USING ERRCODE='55000';
    END IF;
  ...
  IF patched_count<>4 THEN
    RAISE EXCEPTION 'Momence connector admission expected exactly four runtime function guards; patched %',
  ...
  FOREACH function_oid IN ARRAY required_functions LOOP
    ...
    IF position(predecessor_guard IN original_definition)>0
      ...
      RAISE EXCEPTION 'A production Momence runtime function retained an invalid connector guard: %',
        function_oid USING ERRCODE='55000';
    END IF;
  END LOOP;
END
