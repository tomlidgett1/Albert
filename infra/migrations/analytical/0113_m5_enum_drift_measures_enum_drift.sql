-- enum_drift blocked every governed answer whenever a single vendor record
-- sat in canonical quarantine, because its predicate summed unresolved enum
-- drift with the whole open-quarantine backlog and demanded zero, tenant-wide.
--
-- That backlog is already reported, by count, as canonical_mapping_total in
-- the always-included canonical domain. Counting it a second time inside a
-- connector-stream check escalated a warning into a total outage: one
-- un-mappable row in a five-year Lightspeed history disabled the product.
--
-- Measure what the check is named for. Quarantine keeps degrading answers
-- through canonical_mapping_total (warning -> Qualified, never silent), and
-- both counts are now reported separately in the details so the backlog stays
-- visible rather than folded into one opaque number.
--
-- This is not a substitute for draining the backlog: mapper defects that
-- quarantine real sales still under-report those sales and must be fixed.
    ...
    WITH requested_capability AS (
      ...
      SELECT expected.check_id,expected.max_age,
             ...
             CASE
               ...
               ELSE CASE
                 WHEN state.unresolved_enum_drift_count=0
             ...
             jsonb_build_object(
               ...
               'unresolved_enum_drift',coalesce(state.unresolved_enum_drift_count,0),
               'unresolved_quarantine',coalesce(state.unresolved_quarantine_count,0),
