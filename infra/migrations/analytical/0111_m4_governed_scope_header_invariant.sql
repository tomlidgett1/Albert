-- The commerce header invariant compared every order header against its line
-- sum, including sales the vendor has not completed. Lightspeed reports a
-- layby / work-order / quote header total that deliberately exceeds its
-- materialised SaleLines, so 192 zero-line and 5 part-line open sales failed a
-- 0.01 tolerance and took the whole commerce domain -- and therefore every
-- Topic answer -- to unavailable.
--
-- Governed commerce is already defined as completed and not voided in
-- 0110 (tender_reconciles) and 0074 (mart.commerce_sales_event); every metric
-- contract filters order_status=[completed]. Scope the header invariant to the
-- same set so it still fails closed on anything an answer can read, and keep
-- the excluded open-sale drift visible as non_governed_header_drift rather
-- than discarding it. Tolerances are unchanged: completed orders reconcile
-- exactly at 0.0001 on components and 0.01 on headers.
...
DECLARE
  ...
  non_governed_header_failures bigint:=0;
...
BEGIN
  ...
  SELECT count(*) FILTER (WHERE governed),count(*) FILTER (WHERE NOT governed)
    INTO header_failures,non_governed_header_failures
    FROM (
  SELECT order_row.status='completed' AND NOT order_row.voided AS governed
  ...
  INSERT INTO quality.check_result VALUES (
    ...
    jsonb_build_object(
      ...
      'header_scope','completed_not_voided',
      'non_governed_header_drift',non_governed_header_failures,
...
COMMENT ON FUNCTION quality.run_domain_invariants(text,text) IS
  'Runs tenant commerce and finance invariants with one set-based pass per governed relation; the header invariant is scoped to completed, non-voided orders.';
