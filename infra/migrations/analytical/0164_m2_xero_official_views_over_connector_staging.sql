-- Repoints the CubeCore Xero read contract. Every source_xero_official.xo_*
-- view keeps its name and exact output column list but now reads the
-- connector-owned typed staging in source_xero (restored by 0160) instead of
-- the retired dlt XER_OFFICIAL snapshot. Cube models are untouched; only the
-- storage beneath the contract changes. Tenanting moves from the hard-coded
-- tenant_binding row to each staging row's own tenant_id, so the surface
-- becomes genuinely multi-tenant, and every view filters connector
-- tombstones.
--
-- Underlying column types improve where dlt had inferred them (numeric(19,4)
-- instead of double precision / bigint; native date instead of
-- xero_ts(text)::date). CREATE OR REPLACE VIEW refuses type changes, so the
-- old views are dropped and recreated in this one transaction.
--
-- Known mapping gaps, each emitted as a typed NULL and commented at the view:
--   xo_budgets.status, xo_bank_transactions.batch_payment_id,
--   xo_credit_note_allocations.allocation_id (no typed source column exists).
--
-- Apply order is enforced below: this migration refuses to run before 0160,
-- and refuses to swap while source_xero is still empty if the XER_OFFICIAL
-- snapshot is present (i.e. before the first manual ingestion has landed) so
-- Cube can never be flipped onto an empty schema by accident.
...
BEGIN
  IF to_regnamespace('source_xero') IS NULL THEN
    RAISE EXCEPTION 'source_xero staging is missing; apply 0160 first';
  END IF;
  IF to_regnamespace('"XER_OFFICIAL"') IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM pg_stat_user_tables
    WHERE schemaname = 'source_xero' AND n_live_tup > 0
  ) THEN
    RAISE EXCEPTION
      'source_xero holds no rows yet; run the first manual Xero ingestion before repointing the Cube contract';
  END IF;
END $$;
...
FROM "source_xero"."xero_organisations" o
...
FROM "source_xero"."xero_accounts" a
...
FROM "source_xero"."xero_contacts" c
LEFT JOIN "source_xero"."xero_contact_balances" bal
...
FROM "source_xero"."xero_contact_addresses" a
WHERE a.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT a.tombstone;
...
FROM "source_xero"."xero_contact_phones" p
...
FROM "source_xero"."xero_tax_rates" t
...
FROM "source_xero"."xero_currencies" c
WHERE c.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT c.tombstone;
...
FROM "source_xero"."xero_branding_themes" t
WHERE t.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT t.tombstone;

-- MAPPING GAP: xo_budgets.status: neither source_xero.xero_budgets nor the Xero
...
FROM "source_xero"."xero_budgets" g
...
SELECT
  ...
  i."updated_date_utc" AS updated_at
FROM "source_xero"."xero_invoices" i
WHERE i.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT i.tombstone;
...
FROM "source_xero"."xero_invoice_line_items" l
JOIN "source_xero"."xero_invoices" i
...
FROM "source_xero"."xero_payments" p
JOIN "source_xero"."xero_invoices" i
...
FROM "source_xero"."xero_credit_note_allocations" a
JOIN "source_xero"."xero_credit_notes" cn
...
FROM "source_xero"."xero_repeating_invoices" r
...
SELECT
  ...
  l."line_items_account_code" AS account_code,
  l."line_items_item_code"   AS item_code
FROM "source_xero"."xero_repeating_invoice_line_items" l
JOIN "source_xero"."xero_repeating_invoices" r
...
FROM "source_xero"."xero_credit_notes" cn
...
SELECT
  ...
  l."line_items_account_code" AS account_code,
  l."line_items_account_id"   AS account_id
FROM "source_xero"."xero_credit_note_line_items" l
JOIN "source_xero"."xero_credit_notes" cn
...
FROM "source_xero"."xero_credit_note_allocations" a
LEFT JOIN "source_xero"."xero_invoices" i
...
SELECT
  ...
  p."account_account_id" AS account_id,
  p."updated_date_utc" AS updated_at
FROM "source_xero"."xero_batch_payments" p
WHERE p.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT p.tombstone;
...
FROM "source_xero"."xero_payments" m
JOIN "source_xero"."xero_batch_payments" p
...
FROM "source_xero"."xero_overpayments" o
LEFT JOIN "source_xero"."xero_contacts" c
...
SELECT
  ...
  a."amount"         AS amount
FROM "source_xero"."xero_overpayment_allocations" a
LEFT JOIN "source_xero"."xero_invoices" i
  ON i.tenant_id = a.tenant_id
...
-- MAPPING GAP: xo_bank_transactions.batch_payment_id: source_xero.xero_bank_transactions has no
...
FROM "source_xero"."xero_bank_transactions" t
WHERE t.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT t.tombstone;
...
SELECT
  ...
  l.account_code,
  l.account_id
FROM "source_xero"."xero_bank_transaction_line_items" l
JOIN "source_xero"."xero_bank_transactions" t
...
SELECT
  ...
  t.to_is_reconciled,
  t.created_date_utc AS created_at
FROM "source_xero"."xero_bank_transfers" t
WHERE t.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT t.tombstone;

-- Manual journals ------------------------------------------------------------------------------

-- NOTE: source_xero.xero_manual_journals carries no typed DebitTotal/CreditTotal columns; the
...
FROM "source_xero"."xero_manual_journals" j
LEFT JOIN LATERAL (
  SELECT
...
FROM "source_xero"."xero_manual_journal_lines" l
JOIN "source_xero"."xero_manual_journals" j
...
FROM "source_xero"."xero_assets" a
WHERE a.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT a.tombstone;
...
FROM "source_xero"."xero_asset_types" t
WHERE t.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT t.tombstone;
...
SELECT
  ...
  a.object_type,
  a.object_group
FROM "source_xero"."xero_file_associations" a
WHERE a.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT a.tombstone;
...
FROM "source_xero"."xero_payroll_au_employees" e
LEFT JOIN "source_xero"."xero_payroll_au_employee_tax_declarations" td
  ON td.tenant_id = e.tenant_id
 AND td.employee_id = e.employee_id
 AND NOT td.tombstone
LEFT JOIN "source_xero"."xero_payroll_au_employee_home_addresses" ha
...
SELECT
  ...
  p.updated_date_utc AS updated_at
FROM "source_xero"."xero_payroll_au_pay_runs" p
WHERE p.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT p.tombstone;
...
FROM "source_xero"."xero_payroll_au_payslips" s
JOIN "source_xero"."xero_payroll_au_pay_runs" r
...
SELECT
  ...
  t.updated_date_utc AS updated_at
FROM "source_xero"."xero_payroll_au_timesheets" t
WHERE t.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT t.tombstone;
...
FROM "source_xero"."xero_payroll_au_timesheet_lines" l
CROSS JOIN LATERAL jsonb_array_elements(l.number_of_units) WITH ORDINALITY AS u(value, idx)
JOIN "source_xero"."xero_payroll_au_timesheets" t
  ON t.tenant_id = l.tenant_id
...
FROM "source_xero"."xero_payroll_au_earnings_rates" e
WHERE e.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT e.tombstone;
...
FROM "source_xero"."xero_payroll_au_deduction_types" d
...
FROM "source_xero"."xero_payroll_au_leave_types" l
WHERE l.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT l.tombstone;
...
FROM "source_xero"."xero_payroll_au_reimbursement_types" r
WHERE r.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT r.tombstone;
...
FROM "source_xero"."xero_payroll_au_payroll_calendars" c
WHERE c.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT c.tombstone;
...
SELECT
  ...
  f.usi           AS usi,
  f.updated_date_utc AS updated_at
FROM "source_xero"."xero_payroll_au_super_funds" f
WHERE f.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT f.tombstone;
...
FROM "source_xero"."xero_payroll_au_employee_super_memberships" m
WHERE m.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT m.tombstone;
...
FROM "source_xero"."xero_payroll_au_pay_template_earnings_lines" l
WHERE l.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT l.tombstone;
...
SELECT
  ...
  a.code       AS account_code,
  a.name       AS account_name
FROM "source_xero"."xero_payroll_au_settings_accounts" a
WHERE a.tenant_id = (SELECT ingestion.current_tenant_id())
  AND NOT a.tombstone;
...
GRANT SELECT ON
  ...
  source_xero_official.xo_pnl_lines,
  source_xero_official.xo_gst_lines,
...
-- Fail closed: the full contract must exist and nothing may still read the
-- retired snapshot, the tenant binding, or the dlt timestamp shim.
DO $$
...
BEGIN
  ...
  WHERE schemaname = 'source_xero_official'
    AND (definition ~ 'XER_OFFICIAL' OR definition ~ 'tenant_binding' OR definition ~ 'xero_ts');
  IF offender IS NOT NULL THEN
    RAISE EXCEPTION 'repointed views still read retired objects: %', offender;
  END IF;
END $$;
