...
-- Momence staging is connector-owned and may grow as the locked API surface
-- evolves. Extend deletion beneath the signed, one-use wrappers by discovering
-- every base table in source_momence, while preserving the Square and
-- Lightspeed X-Series delegates already composed into the predecessor chain.
ALTER FUNCTION deletion_internal.purge_connection_pre_capability(text,text)
  RENAME TO purge_connection_before_momence_staging;
ALTER FUNCTION deletion_internal.purge_tenant_pre_capability(text)
  RENAME TO purge_tenant_before_momence_staging;
ALTER FUNCTION deletion_internal.verify_connection_pre_capability(text,text)
  RENAME TO verify_connection_before_momence_staging;
ALTER FUNCTION deletion_internal.verify_tenant_pre_capability(text)
  RENAME TO verify_tenant_before_momence_staging;
...
BEGIN
  ...
  -- dependency-safe passes so future Momence child tables cannot strand data.
  FOR pass IN 1..80 LOOP
    ...
    FOR table_row IN
      ...
      WHERE table_value.table_schema='source_momence'
    ...
    LOOP
      IF NOT table_row.has_tenant_id OR NOT table_row.has_connection_id THEN
        RAISE EXCEPTION 'source_momence table % lacks tenant_id or connection_id',
          table_row.table_name
          USING ERRCODE='55000';
      END IF;
      BEGIN
        EXECUTE format(
          'DELETE FROM source_momence.%I
  ...
  LOOP
    ...
    END IF;
    EXECUTE format(
      'SELECT count(*) FROM source_momence.%I AS residual
  ...
  END LOOP;
  IF remaining>0 THEN
    RAISE EXCEPTION 'Momence connection purge left % staging rows',remaining
      USING ERRCODE='55000';
  END IF;

  -- Source rows must be gone before the predecessor removes their immutable
  -- ingestion.batch_manifests parents.
  result:=deletion_internal.purge_connection_before_momence_staging(
...
BEGIN
  ...
  FOR pass IN 1..80 LOOP
    ...
    LOOP
      IF NOT table_row.has_tenant_id THEN
        RAISE EXCEPTION 'source_momence table % lacks tenant_id',
          table_row.table_name
          USING ERRCODE='55000';
      END IF;
      BEGIN
        EXECUTE format(
          'DELETE FROM source_momence.%I WHERE tenant_id=$1',
          table_row.table_name
        ) USING p_tenant_id;
        GET DIAGNOSTICS affected=ROW_COUNT;
        IF affected>0 THEN
  ...
  LOOP
    ...
    END IF;
    EXECUTE format(
      'SELECT count(*) FROM source_momence.%I WHERE tenant_id=$1',
      table_row.table_name
    ) INTO affected USING p_tenant_id;
    remaining:=remaining+affected;
  END LOOP;
  IF remaining>0 THEN
    RAISE EXCEPTION 'Momence tenant purge left % staging rows',remaining
      USING ERRCODE='55000';
  END IF;

  result:=deletion_internal.purge_tenant_before_momence_staging(p_tenant_id);
  RETURN jsonb_set(
    result,
    '{rowsRemoved}',
    to_jsonb(coalesce((result->>'rowsRemoved')::bigint,0)+removed),
...
SET search_path=pg_catalog
AS $$
DECLARE table_row record;row_count bigint;momence_remaining bigint:=0;base jsonb;
BEGIN
  IF NOT core.is_ulid(p_tenant_id) OR NOT core.is_ulid(p_connection_id) THEN
    RAISE EXCEPTION 'deletion scope identifiers must be ULIDs'
      USING ERRCODE='22023';
  END IF;
  PERFORM set_config('albert.tenant_id',p_tenant_id,true);
  base:=deletion_internal.verify_connection_before_momence_staging(
    p_tenant_id,p_connection_id
  );
  ...
  LOOP
    ...
    momence_remaining:=momence_remaining+row_count;
  END LOOP;

  base:=jsonb_set(
    base,
    '{residuals,stagingRows}',
    to_jsonb(coalesce((base#>>'{residuals,stagingRows}')::bigint,0)+momence_remaining),
    false
  );
  base:=jsonb_set(
    base,
    '{remainingRows}',
    to_jsonb(coalesce((base->>'remainingRows')::bigint,0)+momence_remaining),
    false
  );
  RETURN jsonb_set(
    base,
    '{verified}',
    to_jsonb(coalesce((base->>'verified')::boolean,false) AND momence_remaining=0),
    false
  );
END;
$$;
...
BEGIN
  ...
  END IF;
  PERFORM set_config('albert.tenant_id',p_tenant_id,true);
  base:=deletion_internal.verify_tenant_before_momence_staging(p_tenant_id);
...
-- resolves the complete Momence -> Lightspeed X -> Square delegate chain.
...
REVOKE ALL ON FUNCTION
  deletion_internal.purge_connection_before_momence_staging(text,text),
  deletion_internal.purge_tenant_before_momence_staging(text),
  deletion_internal.verify_connection_before_momence_staging(text,text),
  deletion_internal.verify_tenant_before_momence_staging(text),
