...
-- Momence documents no OAuth grant-revocation endpoint. Its deletion target is
-- still evidence-bearing: the deletion worker cryptographically destroys the
-- encrypted credential and records the provider target as unsupported. Keep
-- the validator cumulative with Square and Lightspeed X-Series.
  ...
  LOOP
    IF jsonb_typeof(target) IS DISTINCT FROM 'object'
       ...
       OR target->>'provider' NOT IN (
         'lightspeed-r','lightspeed-x','xero','deputy','square','momence'
       )
