-- Follow-up turns were 503ing for two independent reasons:
-- 1. albert_conversation_history dropped runtime / analyticalRuntime / provider
--    in 0115, so restored Albert v3 (and Grok) threads were classified as
--    OpenAI and hit the immutable runtime lock.
-- 2. Owner follow-ups release the previous lease with fail_albert_turn. Those
--    turns still have a terminal answer event, but model context and history
--    only kept a narrow result_digest allowlist, so the answered turn vanished
