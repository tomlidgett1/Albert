-- ADR 0137: persist the actual rolling 24-hour evidence window.
-- Legacy rows keep NULL windows and are ineligible for the current homepage.
BEGIN;
...
DROP FUNCTION public.albert_save_recommended_analysis(text, integer, jsonb, text, text);
...
BEGIN
  ...
  IF num_nonnulls(p_window_start, p_window_end) = 1
    OR (p_window_end IS NOT NULL AND (
      p_window_end - p_window_start <> interval '24 hours'
      OR p_window_end > clock_timestamp() + interval '5 minutes'
      OR NOT isfinite(p_window_start) OR NOT isfinite(p_window_end)
    ))
    OR (p_model LIKE 'omni:daily-v2:%' AND p_window_end IS NULL) THEN
    RAISE EXCEPTION 'recommended analysis requires its exact 24-hour window' USING ERRCODE = '22023';
  END IF;
