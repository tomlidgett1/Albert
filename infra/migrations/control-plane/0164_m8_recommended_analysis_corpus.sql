-- 0164: recommended analysis becomes a standing brief (ADR 0117).
--
-- Last-four recency is too shallow. The homepage now reads a longer owner
-- history (coverage index + deeper briefs), skips eval/test threads, and
-- caches a short verdict with three next questions.

BEGIN;

ALTER TABLE control_plane.recommended_analysis
  DROP CONSTRAINT IF EXISTS recommended_analysis_source_count_check;
ALTER TABLE control_plane.recommended_analysis
  ADD CONSTRAINT recommended_analysis_source_count_check
  CHECK (source_count BETWEEN 0 AND 40);

ALTER TABLE control_plane.recommended_analysis
  ADD COLUMN IF NOT EXISTS verdict text NOT NULL DEFAULT '';

ALTER TABLE control_plane.recommended_analysis
  DROP CONSTRAINT IF EXISTS recommended_analysis_verdict_check;
ALTER TABLE control_plane.recommended_analysis
  ADD CONSTRAINT recommended_analysis_verdict_check
  CHECK (length(verdict) <= 400);

DROP FUNCTION IF EXISTS public.albert_recent_analysis_briefs(integer);
...
BEGIN
  IF p_limit NOT BETWEEN 1 AND 20 THEN
    RAISE EXCEPTION 'analysis brief limit must be between 1 and 20' USING ERRCODE = '22023';
  END IF;
  ...
  WITH recent AS (
    ...
    WHERE conversation.tenant_id = selected_tenant
      ...
      AND coalesce(conversation.title, '') !~* '^(eval|repro|debug|test)[-:[:space:]]'
      AND NOT EXISTS (
        SELECT 1
  ...
  briefed AS (
    ...
    WHERE length(btrim(coalesce(answer_event.event->>'text', artifact.answer_text, ''))) >= 40
       OR jsonb_typeof(answer_event.event->'keyInsights') = 'array'
       OR jsonb_typeof(answer_event.event->'claims') = 'array'
  )
...
CREATE OR REPLACE FUNCTION public.albert_recommended_analysis_corpus()
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  coverage jsonb;
BEGIN
  briefs := public.albert_recent_analysis_briefs(20);

  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'conversationId', thread.conversation_id,
    'title', coalesce(nullif(btrim(thread.title), ''), thread.question, 'Untitled analysis'),
  ...
  FROM (
    SELECT
      ...
      coalesce((
        SELECT btrim(asked.user_message)
        FROM control_plane.conversation_turns AS asked
        WHERE asked.tenant_id = conversation.tenant_id
          AND asked.conversation_id = conversation.conversation_id
          AND asked.created_by = actor
          AND asked.hidden_at IS NULL
        ORDER BY asked.turn_number DESC
        LIMIT 1
      ), coalesce(conversation.title, '')) AS question
    FROM control_plane.conversations AS conversation
    WHERE conversation.tenant_id = selected_tenant
      AND conversation.created_by = actor
      AND conversation.archived_at IS NULL
...
DROP FUNCTION IF EXISTS public.albert_save_recommended_analysis(text, integer, jsonb, text);
...
BEGIN
  ...
  INSERT INTO control_plane.recommended_analysis (
    tenant_id, user_id, source_fingerprint, source_count, recommendations, model, verdict, generated_at
  ) VALUES (
    selected_tenant, actor, p_source_fingerprint, p_source_count, p_recommendations, btrim(p_model), btrim(p_verdict), clock_timestamp()
  )
...
REVOKE ALL ON FUNCTION public.albert_recommended_analysis_corpus() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.albert_recommended_analysis() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.albert_save_recommended_analysis(text, integer, jsonb, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.albert_recent_analysis_briefs(integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.albert_recommended_analysis_corpus() TO authenticated;
GRANT EXECUTE ON FUNCTION public.albert_recommended_analysis() TO authenticated;
GRANT EXECUTE ON FUNCTION public.albert_save_recommended_analysis(text, integer, jsonb, text, text) TO authenticated;
