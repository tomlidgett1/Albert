...
-- PostgreSQL's LEAST ignores NULL arguments. Require the Auth token expiry
-- explicitly, and bind each grant's row timestamps to one clock sample taken
-- after bounded cleanup so neither volatility nor lock waits can create a
-- grant outside its surviving token/work-lease budget.
...
CREATE OR REPLACE FUNCTION control_plane.issue_raw_storage_webhook_session(
  p_tenant_id text,
...
BEGIN
  ...
  grant_issued_at:=clock_timestamp();
  IF receipt.connector_key='deputy' THEN
    lease_deadline:=grant_issued_at+interval '2 minutes';
  END IF;
  deadline:=least(
    p_auth_token_expires_at,lease_deadline,grant_issued_at+interval '2 minutes'
  );
  IF deadline IS NULL OR deadline<=grant_issued_at+interval '5 seconds' THEN
  ...
  INSERT INTO control_plane.raw_storage_webhook_session_grants(
    ...
    xero_inbox_id,xero_lease_owner,xero_lease_token,xero_lease_version,
    ...
    p_xero_lease_owner,p_xero_lease_token,p_xero_lease_version,
    grant_issued_at,deadline
  );
  RETURN QUERY SELECT generated_id,receipt.tenant_id,receipt.connection_id,expected_key,deadline;
END;
$$;

CREATE OR REPLACE FUNCTION control_plane.issue_raw_storage_deletion_session(
  p_message_id bigint,
...
BEGIN
  ...
  grant_issued_at:=clock_timestamp();
  deadline:=least(
    p_auth_token_expires_at,attempt_deadline,grant_issued_at+interval '5 minutes'
  );
  IF deadline IS NULL OR deadline<=grant_issued_at+interval '5 seconds' THEN
  ...
  INSERT INTO control_plane.raw_storage_deletion_session_grants(
    ...
    request.scope,request.connection_id,grant_issued_at,deadline
  );
