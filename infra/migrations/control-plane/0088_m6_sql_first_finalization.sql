-- Admit the audited sql_first route into answer finalization.
--
-- The per-item route allowlist and the two state rules below were written when
-- model-authored SQL could only ride the source_exploration route, which this
-- function correctly refuses to let support a Verified answer. sql_first is
-- different evidence: the statement's structure was linted against the
-- registry before execution, its claims attested against governed metric
-- contracts after execution, and its answer state derived from the evidence
-- tier of everything touched. Verified may therefore rest on sql_first
-- evidence, while source_exploration remains excluded; an Exploratory answer
-- may now be supported by either route. Everything else is byte-identical to
-- 0074_m6_directory_answer_finalization.sql.
