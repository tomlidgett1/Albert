...
-- The founding employee question offers operational gross profit, not sales,
-- per worked hour. Keep the old option/value row active only so historical
-- overlays remain writable; trusted application code no longer offers its id.
UPDATE control_plane.preference_value_allowlist
   SET option_label='Gross profit'
 WHERE option_id='employee.gross_margin';
    ...
    WHEN 'employee-performance' THEN
      next_overlay:=jsonb_set(
        ...
        to_jsonb(CASE p_option_id
          ...
          WHEN 'profit-per-hour' THEN 'composites.gross_profit_per_labour_hour'
          ELSE 'commerce.net_sales_ex_gst'
        END),true
      );
