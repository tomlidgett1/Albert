-- Admit the pinned Haiku snapshot to the provider-neutral usage ledger and
-- every current metering/finalization guard. No provider payload is stored.
...
BEGIN
  ...
  LOOP
    ...
    IF position('claude-haiku-4-5-20251001' IN definition) > 0 THEN
      CONTINUE;
    END IF;

    updated_definition := replace(
      definition,
      $needle$'grok-4.6',$needle$,
      $replacement$'grok-4.6','claude-haiku-4-5-20251001',$replacement$
    );
