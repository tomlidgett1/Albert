...
-- Scheduled stream jobs already perform connector check_connection before
-- extraction. Connections whose first backfill failed before creating a cursor
-- need the same durable polling path, so the existing incremental scheduler
-- also re-enqueues a recent InitialBackfill coordinator for stale unhealthy
-- connections. This deliberately retains the three locked sync job types.
