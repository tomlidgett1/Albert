...
- `docs/adr/0097-planned-lanes-recipes-and-result-retention.md` — the
  architecture decision record for the changes; `docs/adr/0098-business-context-layer.md`
  for the business context layer (§7).
...
## 7. Business context layer (added after the main loop — ADR 0098)

**What it is.** A ~400-word structured description of the business that every
turn is grounded in — what it is, how it makes money (revenue streams with
shares), scale and seasonality, the owner's goals and vocabulary, and what
each connected tool is the source of truth for. Generated by Albert from the
connected data through config-declared probes (`context_probes`, 20 for the
three test tools; a new connector adds probes, no code), one structured model
call, deterministic rendering (budget 430 words) and a compact classifier
digest; stored per tenant (migration 0153, `control_plane.tenant_business_context`
+ revisions); edited and confirmed by the owner under **About your business**
(edited sections are owner-locked and survive regeneration); refreshed inside
a turn when missing / older than 7 days / a new connector appears (Cube needs
a lease), or on demand ("Rebuild from data", CLI). Injected into every lane's
cached prompt prefix, the classifier, the meta fact sheet and the recipe
composer. Files: `packages/albert-v3/src/context-layer/`,
`app/api/business-context/route.ts`, `app/dash/components/BusinessContextWorkspace.tsx`,
`scripts/albert-business-context.mts`; tests
`tests/contracts/v3-business-context.contract.test.ts` (6).

**Ashburton Cycles instance** (`evals/albert/context/01kzn20vtx2eww1tq2aa3mcpw6.md`,
428 words, generated from 20/20 probes in 78 s and stored): one-location
cycling retailer with a workshop; revenue streams parts & accessories ~55%,
bikes ~24%, workshop/service ~21% (Lightspeed lines, 12 months to Jul 2026);
roughly $600k–$660k a year GST-inclusive, peaking Oct–May with a Jun–Jul
trough; 20 staff on the books / 7 active; 9,613 customers; 1,376 items in
stock worth $161k; watch-points on payables vs receivables and falling workshop
check-ins; vocabulary ("workshop", "the floor", "takings", "wages", "P&L",
"payables", "stock", "GST" → their views/members); tools with data ranges;
cautions from the established source findings.

**Validation** — 42 turns (all 14 ambiguous, 6 new vocabulary questions
V-01…V-06 such as "How's the workshop going?", "How's the floor doing this
week?", "Which side of the business is carrying us — bikes, parts or the
workshop?", 7 meta, 10 cross-tool, 5 others) run with the context off and on,
same engine, same judge (`compare-ctx-off2-vs-ctx-on2.md`; an earlier pair
`compare-ctx-off-vs-ctx-on.md` ran before two classifier fixes it exposed —
see below):

| | context off | context on |
|---|---|---|
| all 42: pass / overall / correctness / detail / format | 57% / 3.64 / 3.98 / 3.31 / 3.95 | 57% / 3.62 / 3.90 / 3.38 / 4.00 |
| ambiguous (20): pass / overall / ambiguity-handling | 65% / 3.85 / 3.79 | **70% / 3.90 / 4.07** |
| vocabulary V-01…06: pass / overall | 50% / 3.33 | **67% / 3.67** |
| clarifications asked on the 20 ambiguous | 0 | 0 |
| cross-tool (10): pass / overall / queries per turn | 56% / 3.44 / 4.6 | 33% / 3.33 / 6.8 |
| latency p50 / p95 (all) | 36 s / 259 s | 32 s / 326 s |

Reading it honestly: the layer does what it is for — vague and vocabulary
questions get the business's own reading without clarification ("the floor" →
retail sales vs the same days last week; "which side is carrying us" → the
three streams by revenue and margin; "the workshop" → jobs, labour and parts
month-to-date vs the same days last month), ambiguity handling and calibrated
detail move up, and the classifier scopes better. It does not lift overall
correctness on this sample, and on cross-tool questions the richer context
nudged the analytical lane to investigate *more* (queries 4.6 → 6.8, more
"over-investigated"). Two fixes came out of the first pair and are in the
tree: "how's X going / doing / tracking / what are X looking like" is always a
comparison with the previous comparable period (never a one-period recipe —
V-01 had gone 745 s → 18 s but too thin), and an unspecified metric is the
business's headline measure, never a clarification (A-05 had started asking).
A third, in the tree but not in the measured numbers: the knowledge block now
says the context is for scoping and phrasing, not a list of angles to
investigate.

**Next steps for the layer.** Entity pre-resolution from the document
(categories, brands, suppliers as `explore_entities` seeds); per-location
documents for multi-site businesses; owner goals used by the reviewer as the
"useful answer" yardstick; and a lighter injection for the analytical lane
(identity + vocabulary + tools only) if the wider-investigation effect persists
on a bigger sample.

## 8. Housekeeping and how to re-run
