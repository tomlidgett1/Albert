...
test("service executes only through semantic read boundary, caches by bundle and never returns SQL",async()=>{
  ...
  const dependencies:SemanticServiceDependencies={registry,contextProvider:{async load(){return{timezone:"Australia/Melbourne",tradingDayCutoff:"00:00",fiscalYearStartMonth:7,fiscalYearStartDay:1,weekStartsOn:1,tenantParameters:{active_customer_days:90,lapsed_customer_days:180,stock_velocity_days:30},capabilities:allCapabilities,overlayVersion:"overlay-1",identityGraphVersion:0,identityGraphHash:"d41d8cd98f00b204e9800998ecf8427e",defaults:{},dossier:{},packVersions:{lightspeed:"1"},sourceWatermarks:{lightspeed:"2026-03-15T11:00:00Z"},authorityByConcept:{operational_sales:"connection-1"}};}},database:{async queryAsSemanticRole(request){databaseCalls+=1;assert.equal(request.tenantId,FIXTURE_TENANT_ID);assert.equal(request.parameters[0],FIXTURE_TENANT_ID);return{rows:[{net_sales_ex_gst:"210.0000"}],durationMs:12};}},cache:new MemorySemanticResultCache(()=>Date.parse(FIXTURE_NOW)),sourceCatalogue:{async listFields(){return[];}},dataHealth:{async getForTopic(){healthCalls+=1;return healthCalls===1?{status:"passed",checks:[{checkId:"line_maths",status:"passed"}]}:{status:"warning",checks:[{checkId:"line_maths",status:"warning"}]};}},audit:{async append(record){audits.push(record);},async promoteSourceField(){return"promotion-1";}},clock:()=>new Date(FIXTURE_NOW)};
  const service=new DefaultSemanticToolExecutor(dependencies);const input={topic:"sales_performance",metrics:["net_sales_ex_gst"],dimensions:[],filters:[],time:{field:"business_date",range:{type:"absolute",from:"2026-03-01T00:00:00.000Z",to:"2026-04-01T00:00:00.000Z"},compare:"none"},sort:[],limit:20,parameters:{}};
...
test("service reloads and recompiles once when identity graph changes before query execution",async()=>{
  ...
  const service=new DefaultSemanticToolExecutor({
    ...
    audit:{async append(){},async promoteSourceField(){return"promotion-1";}},clock:()=>new Date(FIXTURE_NOW),
  });
...
test("source exploration is single-source, allowlisted, PII-gated and parameterised",()=>{
  const catalogue=[
    {connectionId:"connection-1",connectorId:"lightspeed",sourceSchema:"source_lightspeed",sourceTable:"sales",sourceField:"discount_reason",fieldType:"text" as const,piiClass:"business" as const,definition:"Recorded discount reason.",packVersion:"1"},
    {connectionId:"connection-1",connectorId:"lightspeed",sourceSchema:"source_lightspeed",sourceTable:"sales",sourceField:"customer_email",fieldType:"text" as const,piiClass:"customer_contact" as const,definition:"Customer email.",packVersion:"1"}
  ];
