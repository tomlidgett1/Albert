...
test("service reloads and recompiles once when identity graph changes before query execution",async()=>{
  ...
  const response=await service.execute("run_semantic_query",{topic:"sales_performance",metrics:["net_sales_ex_gst"],dimensions:[],filters:[],time:{field:"business_date",range:{type:"absolute",from:"2026-03-01T00:00:00.000Z",to:"2026-04-01T00:00:00.000Z"},compare:"none"},sort:[],limit:20},trusted);
