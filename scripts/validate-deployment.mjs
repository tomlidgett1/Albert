...
assert.equal(contract.schemaVersion, 1, "Deployment contract version is unsupported.");
assert.deepEqual(
...
for (const [file, expected] of Object.entries(services)) {
  ...
  assert.equal(runtime.platform, "fly", `${expected.runtime} must remain a Fly runtime.`);
  assert.equal(runtime.manifest, `deploy/fly/${file}`, `${expected.runtime} manifest drifted.`);
  assert.equal(runtime.artifact, `.albert-build/${expected.command}`, `${expected.runtime} artifact drifted.`);
  assert.equal(runtime.healthPath, "/readyz", `${expected.runtime} readiness path drifted.`);
  assert.ok(Array.isArray(runtime.requiredSecretNames) && runtime.requiredSecretNames.length > 0, `${expected.runtime} must declare required secrets.`);
  assert.equal(new Set(runtime.requiredSecretNames).size, runtime.requiredSecretNames.length, `${expected.runtime} has duplicate required secrets.`);
  assert.equal(new Set(runtime.optionalSecretNames ?? []).size, (runtime.optionalSecretNames ?? []).length, `${expected.runtime} has duplicate optional secrets.`);
  assert.deepEqual(
    runtime.requiredSecretNames.filter((name) => (runtime.optionalSecretNames ?? []).includes(name)),
    [],
  ...
  assert.match(
    body,
    new RegExp(`^\\s*path\\s*=\\s*"${escapeRegularExpression(runtime.livenessPath)}"$`, "mu"),
  ...
  assert.ok(includesAssignment(environment, "ALBERT_CONTROL_PLANE_REGION", "ap-southeast-2"), `${file} must lock the control plane to Sydney.`);
  if (["sync-worker", "transform-worker", "semantic-query", "operator-diagnostic", "deletion-worker"].includes(expected.runtime)) {
    assert.ok(includesAssignment(environment, "ALBERT_ANALYTICAL_REGION", "ap-southeast-2"), `${file} must lock analytics to Sydney.`);
  }
  if (["sync-worker", "webhook-gateway", "deletion-worker"].includes(expected.runtime)) {
    assert.ok(includesAssignment(environment, "SUPABASE_STORAGE_S3_REGION", "ap-southeast-2"), `${file} must lock raw storage to Sydney.`);
  }
  if (expected.runtime === "semantic-query") {
    assert.ok(
      includesAssignment(environment, "ALBERT_MODEL_DATA_RESIDENCY_REGION", "au"),
      `${file} must lock model data residency to AU without claiming regional inference.`,
    );
  }
  assert.equal(environment.includes("ALBERT_SERVICE_VERSION"), false, `${file} must receive its exact release SHA from CI.`);
  assert.equal(environment.includes("ALBERT_DEPLOYMENT_ID"), false, `${file} must receive its deployment ID from CI.`);
  ...
  const serviceSource = await readFile(new URL(`../${expected.source}`, import.meta.url), "utf8");
  assert.ok(serviceSource.includes(runtime.healthPath), `${expected.runtime} does not implement ${runtime.healthPath}.`);
  assert.ok(serviceSource.includes(runtime.livenessPath), `${expected.runtime} does not implement ${runtime.livenessPath}.`);
  if (["sync-worker", "transform-worker"].includes(expected.runtime)) {
    assert.match(body, /^\[metrics\]\s*$/mu, `${file} must publish private Fly metrics.`);
    assert.ok(includesAssignment(section(body, "metrics"), "port", 9091), `${file} metrics port drifted.`);
    assert.match(serviceSource, /queue_(?:depth|oldest_age_seconds)/u, `${expected.runtime} must expose queue pressure.`);
    assert.match(environment, /^\s*ALBERT_WORKER_CONCURRENCY\s*=\s*"[1-9][0-9]*"\s*$/mu, `${file} must declare bounded execution concurrency.`);
  }
}
...
for (const [name, metricPrefix, targetFloor] of [
  ...
  const autoscaler = await readFile(new URL(`../deploy/fly-autoscalers/${name}`, import.meta.url), "utf8");
  assert.match(autoscaler, /^primary_region\s*=\s*"syd"$/mu, `${name} autoscaler must run in Sydney.`);
  assert.match(
    autoscaler,
  ...
  assert.ok(autoscaler.includes(`${metricPrefix}_depth`), `${name} autoscaler must consume queue depth.`);
  assert.ok(autoscaler.includes(`${metricPrefix}_oldest_age_seconds`), `${name} autoscaler must consume queue lag.`);
  assert.ok(autoscaler.includes(`/ ${targetFloor}`), `${name} autoscaler queue-lag target drifted.`);
  assert.match(autoscaler, /FAS_CREATED_MACHINE_COUNT\s*=\s*"min\(40, 2 \+ queue_pressure\)"/u, `${name} autoscaler must retain an HA floor and bounded ceiling.`);
  assert.equal(autoscaler.includes("FAS_API_TOKEN"), false, `${name} autoscaler must not commit its scaling token.`);
  if (name === "transform-worker.toml") {
    assert.ok(autoscaler.includes("albert_transform_maintenance_due"), "Transform autoscaling must include the hourly tenant sweep backlog.");
  }
}

const hosting = JSON.parse(await readFile(new URL("../.openai/hosting.json", import.meta.url), "utf8"));
assert.deepEqual(Object.keys(hosting).sort(), ["d1", "project_id", "r2"], "Sites hosting metadata must not contain runtime configuration.");
...
assert.ok(web.forbiddenRuntimeValues.includes("OPERATOR_DIAGNOSTIC_ANALYTICAL_DATABASE_URL"));
assert.ok(web.forbiddenRuntimeValues.includes("SUPABASE_SERVICE_ROLE_KEY"));
assert.ok(contract.globallyForbiddenRuntimeValues.includes("ANALYTICAL_CAPABILITY_SECRET_BASE64"));

const [packageJson, buildServices, dockerfile, capacityDockerfile, nextConfig,
  webHealth, webDependencyHealth, releaseWorkflow, dogfoodWorkflow, migrationRunner,
  registryPublisher, webProxy, supabaseServer] = await Promise.all([
...
assert.equal(packageJson.scripts["capacity:transform"], "tsx scripts/transform-capacity-harness.ts");
assert.equal(packageJson.scripts["build:capacity-attestor"], "node scripts/build-capacity-attestor.mjs");
assert.match(
  buildServices,
...
assert.match(buildServices, /__ALBERT_SERVICE_BUILD_SHA__:\s*JSON\.stringify\(buildSha\)/u,
  "Service bundles must receive a compile-time code identity.");
assert.match(packageJson.scripts.build, /vinext build/u, "Sites requires the production web build.");
for (const { command } of Object.values(services)) {
  const artifact = command.replace(/^services\//u, "").replace(/\.js$/u, "");
  assert.match(buildServices, new RegExp(`"${escapeRegularExpression(artifact)}"\\s*:`), `Service build is missing ${artifact}.`);
}
assert.match(
  dockerfile,
...
assert.match(dockerfile, /^USER node$/mu, "Service image must run as a non-root user.");
assert.match(webHealth, /inspectWebDependencies/u);
assert.match(webHealth, /status:\s*readiness\.ready\s*\?\s*200\s*:\s*503/u);
assert.match(webHealth, /releaseSha/u, "Sites health must identify the exact release SHA.");
assert.match(nextConfig, /process\.env\.GITHUB_SHA[\s\S]*process\.env\.ALBERT_BUILD_SHA/u);
assert.match(nextConfig, /env:\s*\{[\s\S]*ALBERT_BUILD_SHA:\s*embeddedBuildSha/u);
assert.match(webDependencyHealth, /process\.env\.ALBERT_BUILD_SHA/u,
  "Sites health must report the compile-time bundle identity.");
assert.match(webDependencyHealth, /buildSha === runtimeSha/u,
  "Sites readiness must reject runtime relabelling of an old bundle.");
assert.match(webDependencyHealth, /readiness\.releaseSha === expectedSha[\s\S]*readiness\.deploymentId === expectedDeploymentId/u,
  "Sites readiness must bind dependency code and deployment identity.");
assert.match(webProxy, /"\/reset-password"/u, "Password recovery must pass through the production web guard.");
assert.match(supabaseServer, /assertRuntimeEnvironment\("web"\)/u, "Server-side Supabase access must fail closed on invalid production configuration.");
...
function validateFlyctlPins(workflow, expectedSetups, name) {
  ...
  const setupCount = [...workflow.matchAll(
    /superfly\/flyctl-actions\/setup-flyctl@[^\s]+/gu,
  )].length;
  const pinnedVersions = [...workflow.matchAll(
    new RegExp(
...
assert.match(capacityAttestationJob, /timeout-minutes:\s*90/u,
  "Protected capacity job must retain its reviewed 90-minute cleanup boundary.");
assert.match(capacityAttestationJob,
...
assert.match(releaseWorkflow, /SUPABASE_MANAGEMENT_TOKEN:\s*\$\{\{ secrets\.SUPABASE_MANAGEMENT_TOKEN \}\}/u);
assert.match(
  releaseWorkflow,
...
const capacityRequestStep = releaseWorkflow.slice(capacityRequestStepIndex, capacityRequestStepEnd);
assert.ok(capacityRequestStepIndex > -1, "Protected capacity request step is absent.");
assert.match(capacityRequestStep,
...
assert.match(capacityRequestStep, /STAGING_CELL_ID:[^\n]*ALBERT_CAPACITY_STAGING_CELL_ID/u,
  "Independent attestor request must bind the protected staging cell in its own scope.");
assert.doesNotMatch(releaseWorkflow, /ALBERT_CAPACITY_ED25519_PRIVATE_KEY/u,
...
assert.match(migrationRunner, /sessionUser !== target\.deployerLogin/u, "Release migrations must verify the actual deployer login.");
assert.match(registryPublisher, /publicationRole !== "albert_control_migration_owner"/u, "Registry publication must use the migration owner.");
assert.match(registryPublisher, /session_user !== "albert_control_deployer"/u, "Registry publication must verify the deployer login.");

const [attestorMain, attestorOidc, attestorGithub, attestorCollector, attestorManifest,
  attestorDockerfile, attestorStoreSql, attestorControlSql, attestorAnalyticalSql] = await Promise.all([
  readFile(new URL("../services/capacity-attestor/src/main.ts", import.meta.url), "utf8"),
  readFile(new URL("../services/capacity-attestor/src/oidc.ts", import.meta.url), "utf8"),
  readFile(new URL("../services/capacity-attestor/src/external-observers.ts", import.meta.url), "utf8"),
  readFile(new URL("../services/capacity-attestor/src/collector.ts", import.meta.url), "utf8"),
  readFile(new URL("../deploy/capacity/capacity-attestor.reference.toml", import.meta.url), "utf8"),
  readFile(new URL("../Dockerfile.capacity-attestor", import.meta.url), "utf8"),
  readFile(new URL("../deploy/capacity/attestor-store.sql", import.meta.url), "utf8"),
  readFile(new URL("../deploy/capacity/attestor-control-observer.sql", import.meta.url), "utf8"),
  readFile(new URL("../deploy/capacity/attestor-analytical-observer.sql", import.meta.url), "utf8"),
]);
assert.match(attestorMain, /ALBERT_CAPACITY_ED25519_PRIVATE_KEY_BASE64/u,
  "Independent attestor must own its signer instead of candidate CI.");
assert.match(attestorOidc, /token\.actions\.githubusercontent\.com\/\.well-known\/jwks/u);
assert.match(attestorOidc, /workflow_sha/u, "GitHub OIDC must bind the exact workflow SHA.");
assert.match(attestorGithub, /check_run_id/u, "Independent attestor must bind the active Actions check-run.");
assert.match(attestorGithub, /Only the exact capacity attestation job may enter staging-capacity/u,
  "Independent attestor must reject another protected-environment job.");
for (const source of [attestorGithub, attestorCollector]) {
  assert.match(source, /Prometheus|prometheus/u, "Independent attestor must observe Fly Prometheus.");
  assert.match(source, /Fly|fly/u, "Independent attestor must observe Fly Machines.");
}
assert.match(attestorCollector, /createTransformFleetCapacityAttestation/u,
  "Independent attestor must produce the exact reviewed envelope.");
assert.match(attestorMain, /producerBuildDigest:\s*required\(environment, "ALBERT_CAPACITY_ATTESTOR_BUILD_DIGEST"\)/u,
  "Independent attestor must sign its protected OCI build digest.");
const capacityRequestClient = await readFile(new URL("./request-independent-capacity-attestation.mjs", import.meta.url), "utf8");
assert.match(capacityRequestClient, /response\.status === 202[\s\S]*pollAfterSeconds/u,
  "Capacity request must poll a pending asynchronous observation.");
assert.match(capacityRequestClient, /AbortSignal\.timeout\(20_000\)/u,
  "Every capacity polling exchange must have a short timeout.");
assert.match(attestorManifest, /Reference only[\s\S]*separately reviewed immutable repository/u,
...
const capacityAutoscalerManifest = await readFile(
  new URL("../deploy/capacity/transform-fleet-autoscaler.toml", import.meta.url),
  "utf8",
);
assert.match(
...
process.stdout.write(`validated ${actualFiles.length + 2} runtime deployment boundaries and release preflight\n`);
