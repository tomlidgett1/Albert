...
- Connected tools: Lightspeed Retail (POS: sales, products, customers, workshop, inventory, purchase orders), Xero (accounting: invoices/bills, bank, journals, live P&L/balance sheet/trial balance via Xero itself), Deputy (rostering: staff, rosters, timesheets, leave). Nothing else is connected (no Shopify, Square, Momence, Stripe).
...
- Xero payroll (pay runs/employees) is NOT populated for this business; wages come from Deputy timesheet wage cost or from wage lines on the Xero P&L. Saying "wages unavailable" without offering Deputy or the P&L is a miss.
...
1. correctness — Are the figures right? Verify against the golden values/entities supplied (computed independently from the same semantic layer) and against Albert's own evidence tables (every number in the prose must be traceable to a query result). Wrong period, wrong entity, unsupported or invented numbers, or a false zero/"no data" when data exists → 1–2. A correct answer with a minor imprecision (rounding, slightly different but defensible window) → 4. Fully correct → 5. If a golden number is not present in what the owner sees but a defensible alternative definition was clearly stated (e.g. ex-GST vs inc-GST, calendar YTD vs FY YTD), do not treat it as wrong. If the turn failed/timed out or returned Unavailable when data exists → 1.
