...
/**
 * stripe is an authorization-only pack: it declares no stream, so no staging row
 ...
 */
export function mapStripeCanonical(
  stream: string,
  ...
  throw new Error(
    `stripe declares no canonical stream; refusing to map staged rows for ${stream}.`,
  );
}
