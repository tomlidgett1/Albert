...
- **Walk-ins dominate.** `ls_sales.customer_id = 0` on 38,072 sales — no customer was
  attached. Any per-customer metric covers identified customers only; **say so** in the
  answer. Never collapse the zeros into one phantom mega-customer.
...
- **Marketing flags are negatives.** `no_email = true` means *do not email*. A
  ...
  `ls_contacts`.
- **Store credit and gift cards** live on `ls_credit_accounts` (2,563 rows). Balance
  sign convention: on a Credit Account tender, positive **debits** the account,
  negative **deposits** to it.
- **Customer segments** are `ls_customer_types` (only 2 at this shop) via
  `customer_type_id`.
