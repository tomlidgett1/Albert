...
/**
 * momence is an authorization-only pack: it declares no stream, so no staging row
 ...
 */
export function mapMomenceCanonical(
  stream: string,
  ...
  throw new Error(
    `momence declares no canonical stream; refusing to map staged rows for ${stream}.`,
  );
}
