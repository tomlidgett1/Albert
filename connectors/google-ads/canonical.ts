...
/**
 * google-ads is an authorization-only pack: it declares no stream, so no staging row
 ...
 */
export function mapGoogleAdsCanonical(
  stream: string,
  ...
  throw new Error(
    `google-ads declares no canonical stream; refusing to map staged rows for ${stream}.`,
  );
}
