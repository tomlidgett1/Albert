...
6. **The LLM produces typed semantic plans; deterministic software produces and validates SQL.** There is no agent-facing SQL. The semantic query service is the only analytical SQL path at question time.
...
- **Models:** Claude via the Anthropic API behind a provider abstraction; a small model for intent, clarification decisions and summarisation, a frontier model for planning and synthesis. Prefer AU-region inference endpoints (APP 8).
...
**Initial registry: 44 contracts** (this count is generated; the domains are commerce 13, customers 6, inventory 7, workforce 6, finance 9, composites 3):

- Commerce: gross_takings_inc_gst, net_sales_ex_gst, units_sold, transactions, avg_order_value, items_per_transaction, discount_amount, discount_rate, refund_amount, refund_rate, gross_margin, gross_margin_pct, sell_price_realisation.
...
- Composites: labour_cost_pct_of_sales, sales_per_labour_hour, pos_to_ledger_variance.
...
**Tools:** search_catalogue(question), get_definition(name), get_capabilities(topic), list_field_values(field, query), run_semantic_query(ir), run_source_query(spec), get_data_health(domain), ask_user(question, options), remember(preference), make_chart(spec). remember writes to the tenant overlay only after explicit user confirmation. make_chart renders from returned tables only.

**Loop:** interpret (intent, period, lens, ambiguity) → retrieve (dossier, Topic slice, capabilities, health) → plan (decompose into semantic queries) → clarify only when materially different readings change the answer → compile and execute → independent validation → iterate if evidence requires → answer with provenance and one or two suggested follow-ups. Large result sets go to a summarisation sub-agent with its own context window. Store auditable artefacts per turn: interpreted plan, IR, compiled SQL, result digest, validation outcomes, narrative. Never rely on hidden chain of thought as the record.
