...
import { buildDossierDraft } from "../../services/sync-workers/src/canonical-pipeline.js";
...
test("dossier draft is deterministic, bounded and carries provenance for every fact",()=>{
  const source={
    ...
    sales_tax_basis:"ACCRUALS",tax_number:"GST-registered",xero_observed_at:observedAt,
  ...
  const first=buildDossierDraft(source);
  const second=buildDossierDraft(source);
  assert.ok(first);
...
test("dossier draft does not publish unsupported empty inference",()=>{
  ...
  assert.equal(buildDossierDraft({
    ...
    legal_entity_name:null,base_currency:null,entity_observed_at:null,sales_tax_basis:null,
    tax_number:null,xero_observed_at:null,sale_count:0,sales_observed_at:null,
