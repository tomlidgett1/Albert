...
import { dailyBriefModelLabel, dailyBriefWindow, DAILY_BRIEF_MAX_AGE_MS } from "../../services/recommended-analysis/src/daily-brief";
import type { RecommendedQuestion } from "../../services/recommended-analysis/src/playbook";

const now = new Date("2026-09-10T10:00:00Z");
const row: RecommendedQuestion = { id: "daily-one", title: "Analyse why sales fell 11% this week", question: "What explains the lower sales in the last seven days?", why: "Sales fell 11% against the preceding seven days.", domain: "sales", move: "diagnose", tool: "lightspeed", fromTitle: "Daily look", fromConversationId: null };
...
test("the homepage only serves current observed recommendations from connected sources", () => {
  ...
  for (const cache of [null, { ...cached, model: "playbook" }, { ...cached, model: "omni:gpt-5.6-luna" }, { ...cached, ...dailyBriefWindow(new Date(+now - DAILY_BRIEF_MAX_AGE_MS)) }]) {
