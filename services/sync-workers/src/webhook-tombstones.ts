...
/**
 * Convert pack-declared, verified DELETE identities into source records
 * without a vendor write or a fragile read-after-delete dependency. The exact
 * event body remains attached to webhookReceiptId; this immutable envelope
 * carries only the minimum identity needed to tombstone an existing record.
 */
export function appendVerifiedWebhookTombstones(input: Readonly<{
  ...
  const policy = input.manifest.webhook?.verifiedTombstones;
  const webhookReceiptId = input.job.webhookReceiptId;
  if (!policy || input.job.connectorId !== input.manifest.id ||
      input.job.reason !== policy.jobReason ||
      (policy.requireReceipt && !webhookReceiptId)) {
  ...
  for (const signal of signals) {
    ...
    const payload = Object.freeze({
      schemaVersion: 1,
      signalType: policy.payloadSignalType,
      webhookReceiptId: webhookReceiptId ?? null,
