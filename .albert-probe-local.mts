...
const client = new SemanticServiceClient("http://127.0.0.1:8791", env.ALBERT_SEMANTIC_SIGNING_SECRET!);
const ctx = { tenantId: TENANT, conversationId, turnId, role: "owner" as const };

try {
  const cases = [
    { name: "join-noclaim", sql: "SELECT c.category_name, SUM(f.signed_net_amount_ex_tax) AS net_sales FROM mart.commerce_sales_event f JOIN core.product_category c ON c.category_id = f.rollup_category_id GROUP BY c.category_name", claims: [], time: undefined },
    ...
    { name: "m2-shape", sql: "SELECT w.worker_name, SUM(a.net_sales_ex_gst) AS net_sales, COUNT(DISTINCT a.order_id) AS transactions FROM mart.workforce_sales_aligned a JOIN core.worker w ON w.worker_id = a.worker_id WHERE a.business_date >= '2026-07-08' AND a.business_date < '2026-08-07' GROUP BY w.worker_name ORDER BY net_sales DESC",
      claims: [{ metricId: "commerce.net_sales_ex_gst", column: "net_sales" }], time: { from: "2026-07-08", to: "2026-08-07" } },
    { name: "m1-shape", sql: "SELECT c.category_name, SUM(f.signed_net_amount_ex_tax) AS net_sales, SUM(f.signed_net_amount_ex_tax) - SUM(f.signed_total_cost) AS gross_margin FROM mart.commerce_sales_event f JOIN core.product_category c ON c.category_id = f.rollup_category_id WHERE f.business_date >= '2026-05-09' AND f.business_date < '2026-08-07' GROUP BY c.category_name ORDER BY net_sales DESC",
      claims: [{ metricId: "commerce.net_sales_ex_gst", column: "net_sales" }], time: { from: "2026-05-09", to: "2026-08-07" } },
  ];
  for (const c of cases) {
    try {
      const r: any = await client.execute("run_sql", { sql: c.sql, purpose: "probe " + c.name, claims: c.claims, time: c.time, filters: [], limit: 50 }, ctx);
      console.log(JSON.stringify({ name: c.name, state: r.state, rows: (r.data?.rows ?? []).length, checks: (r.validation?.checks ?? []).filter((x: any) => x.status !== "passed").map((x: any) => x.checkId + "=" + x.status) }));
    } catch (error: any) {
