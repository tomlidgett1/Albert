...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      if (threeRef.current) {
        const t = threeRef.current as ThreeRuntime & {
      ...
      const applySize = () => {
        ...
        // Composer must use CSS pixels; buffer size (x DPR) blows up the canvas.
      ...
      const setSize = () => {
        if (sizeRaf) return;
        // Double-rAF avoids ResizeObserver loop errors from sync canvas writes.
        sizeRaf = requestAnimationFrame(() => {
          sizeRaf = requestAnimationFrame(applySize);
        });
      };
      applySize();
      // Window resize only: ResizeObserver + WebGL setSize can trip Chrome's
      // "ResizeObserver loop completed with undelivered notifications" overlay.
      window.addEventListener("resize", setSize, { passive: true });
      const ro = {
        disconnect: () => window.removeEventListener("resize", setSize),
        observe: () => undefined,
      ...
      let touch: TouchTextureController | undefined;
      ...
      const runtime = threeRef.current as ThreeRuntime & {
        intersectionObserver?: IntersectionObserver;
        sizeRaf?: number;
        cancelSizeRaf?: () => void;
      };
      runtime.intersectionObserver = io;
      runtime.cancelSizeRaf = () => {
