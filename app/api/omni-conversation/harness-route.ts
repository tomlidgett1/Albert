...
import {
  ALBERT_OAI_CODEX_ANALYTICAL_RUNTIME,
  ALBERT_OAI_CODEX_DEFAULT_EFFORT,
  ALBERT_OAI_CODEX_DEFAULT_FAST_MODE,
  ALBERT_OAI_CODEX_DEFAULT_MODEL,
  ALBERT_OAI_CODEX_HARNESS,
  ALBERT_OAI_CODEX_MODEL_IDS,
  ALBERT_OAI_CODEX_RUNTIME,
  ALBERT_OAI_CODEX_RUNTIME_HEADER,
} from "@/packages/albert-oai-codex/src";
import {
...
/**
 * One conversation route body for every Omni-style harness (ADR 0141). The
 * Omni and OAI Codex routes differ only in the profile below: which agent
 * loop the runtime service runs, which models it admits, and the identity
 * stamped on the conversation and its stream. Everything else — auth,
 * routing of social/direct messages, context loading, the governed trace
 * relay — is shared so a comparison between harnesses is a comparison of
 * harnesses.
 */
export type OmniHarnessRouteProfile = Readonly<{
  harness: OmniHarness;
  /** Runtime profile stamped on the conversation turn (runtime lock). */
  runtime: string;
  analyticalRuntime: string;
  /** The `X-Albert-Runtime` value the dash matches against its selection. */
  runtimeHeader: string;
  modelIds: readonly string[];
  unsupportedModelMessage: string;
  defaultPreferences: AgentRunPreferences;
  /** Logger scope and event prefix ("albert-omni-web" / "omni"). */
  logScope: string;
  logPrefix: string;
  /** Turn failure-code prefix ("albert_omni" → albert_omni_answered, …). */
  failureCodePrefix: string;
  unavailableMessage: string;
  startFailedMessage: string;
}>;

export const OMNI_ROUTE_PROFILE: OmniHarnessRouteProfile = Object.freeze({
  harness: "omni",
  ...
  logScope: "albert-omni-web",
  logPrefix: "omni",
  failureCodePrefix: "albert_omni",
  unavailableMessage: "The Omni runtime is not configured on this environment.",
  startFailedMessage: "The Omni conversation could not be started.",
});

export const OAI_CODEX_ROUTE_PROFILE: OmniHarnessRouteProfile = Object.freeze({
  harness: ALBERT_OAI_CODEX_HARNESS,
  runtime: ALBERT_OAI_CODEX_RUNTIME,
  ...
  defaultPreferences: Object.freeze({
    ...
    fastMode: ALBERT_OAI_CODEX_DEFAULT_FAST_MODE,
  }),
  logScope: "albert-oai-codex-web",
  logPrefix: "oai_codex",
  failureCodePrefix: "albert_oai_codex",
  unavailableMessage: "The OAI Codex harness is not configured on this environment.",
  startFailedMessage: "The OAI Codex conversation could not be started.",
});
...
function publicHarnessFailure(error: unknown, profile: OmniHarnessRouteProfile): string {
  if (error instanceof OmniRuntimeServiceError) {
    const byCode: Readonly<Record<string, string>> = {
      omni_unavailable: profile.unavailableMessage,
      omni_runtime_unavailable: "The analysis runtime is unavailable right now.",
...
export async function handleOmniHarnessConversation(
  request: Request,
  profile: OmniHarnessRouteProfile,
): Promise<Response> {
  const logger = createServiceLogger(profile.logScope);
  const failureCode = (suffix: string) => `${profile.failureCodePrefix}_${suffix}`;
  const logEvent = (suffix: string) => `${profile.logPrefix}.${suffix}`;
  ...
  const preferences = normalizeAgentPreferences(parsed.preferences ?? profile.defaultPreferences);
  if (!profile.modelIds.includes(preferences.model)) {
    return jsonError(profile.unsupportedModelMessage, 400, correlationId);
  }
  if (
  ...
  if (!cubeApiSecret || !cubeApiUrl || !serviceUrl || !serviceSigningSecret) {
    logger.error(logEvent("configuration_missing"), {
    ...
    return jsonError(profile.unavailableMessage, 503, correlationId);
  }
  ...
  const runtimeProfile = {
    ...
    analyticalRuntime: profile.analyticalRuntime,
  ...
  try {
    begun = await beginConversationTurn({
      ...
      staleLeaseFailureCode: failureCode("stale_lease_released"),
      supabase: auth.supabase,
    });
  } catch (error) {
    const status = error instanceof ControlPlaneError ? error.status : 503;
    logger.error(logEvent("turn_begin_failed"), { status, ...safeErrorEvidence(error) }, correlationId);
    return jsonError(profile.startFailedMessage, status, correlationId);
  }
    ...
    const response = createLiveTraceSseResponse({
      ...
      run: async (stream) => {
        const emit = createTraceEmitter({
          ...
          deliver: stream.emit,
          onPersistError: (error, event) => logger.warn(logEvent("local_conversation_trace_persist_failed"), {
            conversationId,
            turnId,
        ...
        try {
          ...
          await failConversationTurn({
            conversationId,
            turnId,
            failureCode: localFailureCode,
            supabase: auth.supabase,
          });
        } catch (error) {
          logger.error(logEvent("local_conversation_turn_failed"), safeErrorEvidence(error), correlationId);
          await failConversationTurn({
            conversationId,
            turnId,
            failureCode: failureCode("local_conversation_failure"),
            supabase: auth.supabase,
          }).catch(() => undefined);
        }
      },
    });
    response.headers.set("X-Albert-Runtime", profile.runtimeHeader);
  ...
  if (socialKind) {
    ...
    return localConversationResponse(reply.text, reply.followUps, failureCode("social_answered"));
  }
  ...
  if (directConversationReply) {
    return localConversationResponse(directConversationReply, [], failureCode("direct_conversation_answered"));
  }
    ...
    if (routerApiKey) {
      const decision = await routeCodexMessage({
        ...
        if (!request.signal.aborted) {
          logger.warn(logEvent("message_router_failed"), safeErrorEvidence(error), correlationId);
        }
        return { route: "analysis" as const, response: null };
      });
      if (decision.route === "conversation" && decision.response) {
        return localConversationResponse(decision.response, [], failureCode("routed_conversation_answered"));
      }
    }
  }
  ...
  const initialAcknowledgement = acknowledgementApiKey
    ? generateInitialAcknowledgement({
        ...
        if (!request.signal.aborted) {
          logger.warn(logEvent("initial_acknowledgement_failed"), {
            conversationId,
            turnId,
            ...safeErrorEvidence(error),
          }, correlationId);
  ...
  logger.info(logEvent("turn_started"), {
  ...
  const response = createLiveTraceSseResponse({
    ...
    run: async (stream, streamSignal) => {
      ...
      const emit = createTraceEmitter({
        ...
        deliver: stream.emit,
        onPersistError: (error, event) => logger.warn(logEvent("trace_event_persist_failed"), {
          conversationId,
          turnId,
          eventType: event.type,
          ...safeErrorEvidence(error),
      ...
      void (async () => {
        try {
          ...
          logger.warn(logEvent("title_generation_failed"), safeErrorEvidence(error), correlationId);
        }
      })();
      try {
        ...
        const serviceTurn: OmniServiceTurn = {
          ...
          ...(parsed.dashboardBuild && parsed.dashboardEdit && parsed.dashboardEditTopic
            ...
            : {}),
          // The default harness travels implicitly so a runtime deployed
          // before ADR 0141 keeps accepting Omni turns.
          ...(profile.harness === "omni" ? {} : { harness: profile.harness }),
        };
        ...
        const deliverRuntimeEvent = async (event: OmniTraceEventInput) => {
          try {
            ...
            logger.error(logEvent("trace_event_rejected"), {
              conversationId,
              turnId,
              eventType: event.type,
              ...safeErrorEvidence(error),
        ...
        await failConversationTurn({
          conversationId,
          turnId,
          failureCode: result.answerState === "Unavailable" ? failureCode("unavailable") : failureCode("answered"),
          supabase: auth.supabase,
        });
        logger.info(logEvent("turn_completed"), {
          tenantId: tenant.tenant_id,
          conversationId,
          turnId,
          harness: result.harness ?? profile.harness,
        ...
        logger.error(disconnected ? logEvent("turn_disconnected") : logEvent("turn_failed"), {
          tenantId: tenant.tenant_id,
          conversationId,
          turnId,
          harness: profile.harness,
        ...
        try {
          if (!disconnected) {
            await emit({
              type: "error",
              status: "error",
              message: publicHarnessFailure(error, profile),
              recoverable: true,
            });
          }
          await emit.drain?.();
          ...
          logger.error(logEvent("turn_finalize_failed"), safeErrorEvidence(finalizeError), correlationId);
        }
      } finally {
  ...
  response.headers.set("X-Albert-Harness", profile.harness);
  response.headers.set("X-Request-Id", correlationId);
  return response;
}
