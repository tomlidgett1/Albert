...
export type CubeQueryAuditContext = Readonly<{
  operation?: string;
  topic?: string;
  branchLabel?: string;
}>;
...
export class CubeClient {
  ...
  async loadQuery(
    query: CubeQuery,
    options: Readonly<{ signal?: AbortSignal; audit?: CubeQueryAuditContext }> = {},
    ...
    try {
      ...
      await finishAnalyticalQueryAttempt(this.queryRecorder, begun, {
        ...
        failureCode: options.signal?.aborted ? "cube_query_cancelled" : "cube_query_exception",
        failureMessage: sanitizeQueryFailureMessage(error, "Cube could not execute the governed query."),
      });
      throw error;
    }
    ...
    if (loaded.result.ok) {
      ...
      const rejected = loaded.validated === undefined
        && !/down|could not be reached|HTTP 5|timed out|timeout|cancelled|aborted/iu.test(loaded.result.error);
      await finishAnalyticalQueryAttempt(this.queryRecorder, begun, {
        status: cancelled ? "cancelled" : rejected ? "rejected" : "failed",
        executionMs: loaded.result.executionMs,
        failureCode: normalizeQueryFailureCode(
          cancelled ? "cube_query_cancelled" : rejected ? "cube_query_rejected" : "cube_query_failed",
        ),
