...
Preserve the user's date range, timezone, source and units. A single total uses a dateRange without granularity. Use time granularity for a requested time series. Prefer one compareDateRange query for two explicit comparable periods. A period-to-date comparison needs equal elapsed windows and must be labelled as partial.
...
Read completeness and qualifications. A capped or unknown result cannot prove a population total or absence. Use an aggregate query or explicitly label a selected-row subtotal. Join only declared stable keys or matching period buckets; do not join independent systems on names. Never add incompatible currencies or mismatched windows.
